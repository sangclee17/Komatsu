...
class ProjectEdge(basedialog.BaseDialog):
    ...
    def CreateWidgets(self):
        ...
        tk.Label(self.frmTop, text='SimLab画面からエッジと投影面を選択してください。').pack(side=tk.TOP, anchor=tk.CENTER)
        ...
        self.btnExcute = tk.Button(self.frmCtrl, text=' 実行 ', compound=tk.LEFT, command=self.ImprintEdge, width=10)
    ...
    def ImprintEdge(self):
        ...
        if not faces:
            messagebox.showinfo('情報','投影面を選択してください。')
            return
