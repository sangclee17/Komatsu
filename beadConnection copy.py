...
class ConnectBeadDialog(basedialog.BaseDialog):
    ...
    def CreateWidgets(self):
        ...
        self.angleBeadFaceGroup = 'angleBeadFace'
        self.angleNodeGroup = 'angleNode'
        simlablib.DeleteGroups([self.angleBeadFaceGroup, self.angleNodeGroup])
    ...
    def SelectAngleEndBead(self):
        face = simlab.getSelectedEntities('FACE')
        if len(face) == 0:
            messagebox.showinfo('情報','面を選択した後、ボタンを押してください。')
            return
        if len(face) != 1:
            messagebox.showinfo('情報','一つの面を選択してください。')
            return
        
        simlablib.DeleteGroups(self.angleBeadFaceGroup)

        modelName = simlab.getModelName('FEM')
        simlablib.CreateGroup(modelName, 'Face', face, self.angleBeadFaceGroup)
    ...
    def ConnectAngle(self):
        ...
        nodeEquivalence(modelName, beadCreated)
        remeshFace(modelName, beadFaces, meshSize, 10, preserveBoundary=1)
    ...
    def ConnectAngle1(self):
        beadEndFace = simlab.getEntityFromGroup(self.angleBeadFaceGroup)
        if not beadEndFace:
            messagebox.showinfo('情報','ビード表面を選択してください。')
            return
        
        nodeInside = simlab.getEntityFromGroup(self.angleNodeGroup)
        if not nodeInside:
            messagebox.showinfo('情報','ノードを選択してください。')
            return
        ...
        faceNormal = simlab.getNormalFromFaceGroup(self.angleBeadFaceGroup)
        if not faceNormal:
            messagebox.showinfo('情報','ビード表面として平面を選択してください。')
            return
        ...
        bodyGroup = 'bodyGroup'
        simlablib.DeleteGroups(bodyGroup)
        simlablib.SelectAssociatedEntities(modelName, 'Face', beadEndFace, 'Body', bodyGroup)
        beadEndBody = list(simlab.getBodiesFromGroup(bodyGroup))
        
        simlablib.DeleteGroups(edgeGroup)
        simlablib.SelectAssociatedEntities(modelName, 'Face', beadEndFace, 'Edge', edgeGroup)
        beadEdges = simlab.getEntityFromGroup(edgeGroup)
        splitEdgeByAngle(modelName, beadEdges)
        beadEdges = simlab.getEntityFromGroup(edgeGroup)

        simlablib.DeleteGroups(vertexGroup)
        simlablib.SelectAssociatedEntities(modelName, 'Face', beadEndFace, 'Vertex', vertexGroup)
        beadvertexes = simlab.getEntityFromGroup(vertexGroup)
        if len(beadvertexes) < 3:
            messagebox.showinfo('情報','ビード表面の節点が３以下です。')
            return
        ...
        sharedBeadVertexNodes = []
        faceGroup = 'faceGroup'
        for thisV in beadVertexNodes:
            simlablib.DeleteGroups(faceGroup)
            simlablib.SelectAssociatedEntities(modelName, 'Node', [thisV], 'Face', faceGroup)
            assFaces = simlab.getEntityFromGroup(faceGroup)
            if len(assFaces) > 1:
                sharedBeadVertexNodes.append(thisV)
        
        if len(sharedBeadVertexNodes) < 3:
            messagebox.showinfo('情報', 'ビード表面の共有節点が３以下です。')
            return

        #print('sharedBeadVertexNodes:{}'.format(sharedBeadVertexNodes))

        if not nodeInside[0] in sharedBeadVertexNodes:
            messagebox.showinfo('情報','ビード表面の節点ノードを選択してください。')
            return

        refNode = nodeInside[0]
        sharedBeadVertexNodes.remove(refNode)
        node0Pt = np.array(simlab.getNodePositionFromNodeID(modelName, refNode))
        vertexNodeDistDict = dict()
        for thisNode in sharedBeadVertexNodes:
            thisNodePt = np.array(simlab.getNodePositionFromNodeID(modelName, thisNode))
            dist = np.linalg.norm(thisNodePt - node0Pt)
            vertexNodeDistDict[thisNode] = dist
        
        sortedBeadVertex = sorted(vertexNodeDistDict.items(), key=lambda x:x[1])
        nodeTuple1, nodeTuple2 = sortedBeadVertex[-2:]
        node1 = nodeTuple1[0]
        node2 = nodeTuple2[0]
        ...
        bodiesToMerge = []
        beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        createNonSharedBodyFromFace(modelName, beadEndFace)
        afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        newBody = list(set(afterBodies) - set(beforeBodies))
        if not newBody:
            ...
            bodiesToMerge.extend(newBody)
        
        beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        rotateBody(modelName, newBody, list(node0Pt), list(node1Pt))
        afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        newBody1 = list(set(afterBodies) - set(beforeBodies))
        if newBody1:
            faceGroup = 'faceGroup'
            simlablib.DeleteGroups(faceGroup)
            simlablib.SelectAssociatedEntities(modelName, 'Body', newBody1, 'Face', faceGroup)
            bodyFaces = simlab.getEntityFromGroup(faceGroup)
            simlablib.DeleteGroups(faceGroup)
            simlablib.CreateGroup(modelName, 'Face', bodyFaces, faceGroup)
            n1 = np.array(simlab.getNormalFromFaceGroup(faceGroup))
            d1 = np.dot(n1, node0Pt)
            if np.dot(n1, node2Pt) - d1 > 0:
                simlablib.DeleteEntities(modelName, 'Body', newBody1)
                beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
                rotateBody(modelName, newBody, list(node1Pt), list(node0Pt))
                afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
                newBody1_retry = list(set(afterBodies) - set(beforeBodies))
                if newBody1_retry:
                    bodiesToMerge.extend(newBody1_retry)
            else:
                bodiesToMerge.extend(newBody1)
        
        beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        rotateBody(modelName, newBody, list(node0Pt), list(node2Pt))
        afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        newBody2 = list(set(afterBodies) - set(beforeBodies))
        if newBody2:
            faceGroup = 'faceGroup'
            simlablib.DeleteGroups(faceGroup)
            simlablib.SelectAssociatedEntities(modelName, 'Body', newBody2, 'Face', faceGroup)
            bodyFaces = simlab.getEntityFromGroup(faceGroup)
            simlablib.DeleteGroups(faceGroup)
            simlablib.CreateGroup(modelName, 'Face', bodyFaces, faceGroup)
            n2 = np.array(simlab.getNormalFromFaceGroup(faceGroup))
            d2 = np.dot(n2, node0Pt)
            if np.dot(n2, node1Pt) - d2 > 0:
                simlablib.DeleteEntities(modelName, 'Body', newBody2)
                beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
                rotateBody(modelName, newBody, list(node2Pt), list(node0Pt))
                afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
                newBody2_retry = list(set(afterBodies) - set(beforeBodies))
                if newBody2_retry:
                    bodiesToMerge.extend(newBody2_retry)
            else:
                bodiesToMerge.extend(newBody2)
        if bodiesToMerge:
            simlablib.UpdateModel()
            beadName = getUniqueBeadName(modelName)
            mergeBodies(modelName, bodiesToMerge, beadName)
            nodeEquivalence(modelName, beadName)
            fillFreeEdgesOfBody(modelName, beadName)

            remeshOn = 1 if remeshChecked else 0
            ...
            adjBodies = simlab.getBodiesFromGroup(adjBodyGroup)
            for thisBody in adjBodies:
                joinOverlapFaces(modelName, beadName, thisBody, remesh=remeshOn)

            if bodyMergeChecked:
                beadEndBody.append(beadName)
                simlablib.MergeBodies(modelName, beadEndBody)
    ...
    def ConnectCurve(self):
        ...
        beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        createStraightEdgeFromNode(modelName, beadFaceCylinderIntersectingNodeDict[beadFaceKeys[0]], beadFaceCylinderIntersectingNodeDict[beadFaceKeys[1]])
        afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        edgeBody = list(set(afterBodies) - set(beforeBodies))
        if not edgeBody:
            messagebox.showinfo('情報','エッジ作成にエーラが発生しました。')
    ...
    def ConnectStraight(self):
        ...
        if len(edge1Nodes) !=2:
            ...
            return
        beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        createStraightEdgeFromNode(modelName, edge1Nodes[0], edge1Nodes[1])
        afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        edgeBody = list(set(afterBodies) - set(beforeBodies))
        if not edgeBody:
            messagebox.showinfo('情報','エッジ作成にエーラが発生しました。')
        ...
        beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        createStraightEdgeFromNode(modelName, edge2Nodes[0], edge2Nodes[1])
        afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        edgeBody = list(set(afterBodies) - set(beforeBodies))
        if not edgeBody:
            messagebox.showinfo('情報','エッジ作成にエーラが発生しました。')
        ...
        beforeBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        createStraightEdgeFromNode(modelName, edge3Nodes[0], edge3Nodes[1])
        afterBodies = simlab.getBodiesWithSubString(modelName, ['*'])
        edgeBody = list(set(afterBodies) - set(beforeBodies))
        if not edgeBody:
            messagebox.showinfo('情報','エッジ作成にエーラが発生しました。')
